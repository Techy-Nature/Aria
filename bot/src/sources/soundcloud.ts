...
interface Token { access_token: string; expires_in: number; refresh_token?: string }
interface ScTrack { urn?: string; id?: number; title: string; duration?: number; permalink_url: string; artwork_url?: string; access?: "playable" | "preview" | "blocked"; streamable?: boolean; user: { username: string; permalink_url?: string } }
...
export class SoundCloudProvider implements MediaProvider {
  ...
  private token?: { value: string; expiresAt: number; refreshToken?: string }; private refreshing?: Promise<string>;
  ...
  private track(value: ScTrack): Track {
    const providerId = value.urn ?? (value.id === undefined ? undefined : String(value.id));
    if (!providerId) throw new ProviderUnavailableError("SoundCloud returned a track without an identifier.");
    return { id: value.urn ?? `soundcloud:legacy:${providerId}`, providerId, provider: "soundcloud", playable: value.access !== "blocked" && value.streamable !== false, title: value.title, artist: value.user.username, duration: Math.round((value.duration ?? 0) / 1000), artwork: value.artwork_url, url: value.permalink_url, webUrl: value.permalink_url, attribution: { service: "SoundCloud", uploaderUrl: value.user.permalink_url } };
  }
  ...
  async resolve(track: Track, positionSeconds = 0): Promise<PlayableMedia> {
    ...
    const current = await this.api<ScTrack>(`/tracks/${encodeURIComponent(resourceId)}`);
    ...
    const streams = await this.api<Streams>(`/tracks/${encodeURIComponent(resourceId)}/streams`); const inputUrl = streams.hls_aac_160_url ?? streams.hls_aac_96_url;
