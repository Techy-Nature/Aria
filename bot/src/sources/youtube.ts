...
export const MAX_PLAYLIST_TRACKS = 150;
export const MAX_PLAYLIST_PAGES = 3;
...
export class YouTubeProvider implements MediaProvider {
  ...
  async search(query: string, limit: number, playableOnly = false): Promise<Track[]> {
    ...
    url.search = new URLSearchParams({ part: "snippet", type: playableOnly ? "video" : "video,playlist", maxResults: String(limit), q: query, key: this.apiKey }).toString();
  ...
  private async playlist(playlistId: string): Promise<Track[]> {
    ...
    const tracks: Track[] = []; let pageToken: string | undefined, pages = 0;
    do {
    ...
    } while (pageToken && pages < MAX_PLAYLIST_PAGES && tracks.length < MAX_PLAYLIST_TRACKS);
  ...
  async fromUrl(input: string): Promise<Track | Track[]> {
    ...
    if (url.pathname.replace(/\/+$/, "") === "/playlist" && playlistId) return this.playlist(playlistId);
