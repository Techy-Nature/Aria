...
const players = new PlayerManager(); const search = new SearchService(); const feedback = new PlaybackFeedback(); const router = new CommandRouter(new SettingsStore(process.env.ARIA_PREFIX), players, search, undefined, feedback); const adapter = createAdapter();
const playback = adapter.voice ? new PlaybackCoordinator(players, adapter.voice, feedback) : undefined;
