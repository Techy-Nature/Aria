...
export class FluxerVoiceTransport extends LiveKitVoiceTransport {
  ...
  protected async credentials(guildId: string, channelId: string) { const assignment = this.gateway.waitForVoiceServer(guildId, channelId); this.gateway.requestVoice(guildId, channelId); const result = await assignment; this.connectionIds.set(guildId, result.connectionId); return result; }
