...
export class FluxerAdapter implements PlatformAdapter {
  ...
  private voice = new Map<string, string>();
  ...
  handleGatewayEvent(value: unknown) {
    ...
    if (packet.op === 12) { const d = record(packet.d); console.error(`[Fluxer] Gateway error${d?.code ? ` ${String(d.code)}` : ""}: ${String(d?.message ?? "unknown")}`); return; }
    ...
    if (packet.t === "VOICE_STATE_UPDATE") { const uid = text(d.user_id), cid = text(d.channel_id); if (uid) cid ? this.voice.set(uid, cid) : this.voice.delete(uid); return; }
  ...
  private async normalizeMessage(message: Record<string, unknown>) {
    ...
    await this.handler({ guildId, channelId, userId, voiceChannelId: this.voice.get(userId), reply: async (reply, components) => { await this.sendMessage(channelId, reply, components); } }, content);
  }
  ...
  async stop() { this.stopped = true; this.clearHeartbeat(); if (this.reconnect) clearTimeout(this.reconnect); this.reconnect = undefined; const socket = this.socket; this.socket = undefined; socket?.close(1000, "Aria shutting down"); console.log("[Fluxer] Stopped"); }
}
