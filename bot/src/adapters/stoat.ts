...
import { StoatVoiceTransport } from "../voice/stoat.js";
...
export class StoatAdapter implements PlatformAdapter {
  readonly voice: StoatVoiceTransport;
  ...
  private channels = new Map<string, string>(); private memberVoice = new Map<string, string>();
  constructor(private token: string, private fetcher: Fetch = fetch, private sockets: SocketFactory = defaultSocketFactory) { this.voice = new StoatVoiceTransport(token, fetcher as typeof fetch); }
  ...
  handleEvent(value: unknown) {
    ...
    if (event.type === "VoiceChannelJoin") { const state = record(event.state), uid = text(state?.id), cid = text(event.id); if (uid && cid) this.memberVoice.set(uid, cid); return; }
    if (event.type === "VoiceChannelLeave") { const uid = text(event.user); if (uid) this.memberVoice.delete(uid); return; }
    if (event.type === "VoiceChannelMove") { const uid = text(event.user), cid = text(event.to); if (uid && cid) this.memberVoice.set(uid, cid); return; }
  ...
  private loadReady(event: Record<string, unknown>) {
    ...
    if (Array.isArray(event.voice_states)) for (const item of event.voice_states) { const state = record(item), cid = text(state?.id); if (cid && Array.isArray(state?.participants)) for (const p of state.participants) { const participant = record(p), uid = text(participant?.id); if (uid) this.memberVoice.set(uid, cid); } }
  }
  private async normalizeMessage(message: Record<string, unknown>) {
    ...
    await this.handler({ guildId, channelId, userId, voiceChannelId: this.memberVoice.get(userId), reply: async reply => { await this.sendMessage(channelId, reply); } }, content);
  }
  ...
  async stop() { await this.voice.shutdown(); this.stopped = true; this.clearPing(); if (this.reconnect) clearTimeout(this.reconnect); this.reconnect = undefined; const socket = this.socket; this.socket = undefined; socket?.close(1000, "Aria shutting down"); console.log("[Stoat] Stopped"); }
}
