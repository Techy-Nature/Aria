  ...
  async handle(ctx: CommandContext, content: string) {
    ...
    try {
      switch (command) {
        ...
        case "play": case "enqueue": { const query = args.join(" "); if (!query) return ctx.reply("Provide a song, playlist, or link."); if (!ctx.voiceChannelId) return ctx.reply("Join a voice channel first."); const currentVoice = this.players.get(ctx.guildId).voiceChannelId; if (currentVoice && currentVoice !== ctx.voiceChannelId) return ctx.reply("Join Aria's current voice channel first."); const tracks = await this.search.search(query, 1, true); if (!tracks[0]) return ctx.reply("That search is metadata-only; configure a playable provider or provide a direct media URL."); if (tracks.some(track => track.playable === false)) return ctx.reply("That playlist contains search-only results and can't currently be played."); this.feedback?.associate(ctx.guildId, ctx.reply); this.players.setVoiceChannel(ctx.guildId, ctx.voiceChannelId); this.players.add(ctx.guildId, tracks, command === "enqueue"); const count = tracks.length > 1 ? ` and ${tracks.length - 1} more tracks` : ""; return ctx.reply(`${command === "enqueue" ? "Playing next" : "Queued"}: **${tracks[0].title}**${count}.`); }
