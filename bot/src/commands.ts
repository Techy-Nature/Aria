  ...
  async handle(ctx: CommandContext, content: string) {
    ...
    try {
      switch (command) {
        ...
        case "play": case "enqueue": { const query = args.join(" "); if (!query) return ctx.reply("Provide a song, playlist, or link."); const tracks = await this.search.search(query, 1); this.players.add(ctx.guildId, tracks, command === "enqueue"); this.players.get(ctx.guildId).voiceChannelId = ctx.voiceChannelId; return ctx.reply(`${command === "enqueue" ? "Playing next" : "Queued"}: **${tracks[0].title}**.`); }
